...
2. **Python venv** - creates `.venv` at the repo root if missing (shared across `statement-loader` and `scripts/sql/finance_manager_db`, matching the existing `scripts/python_setup` convention), then `pip install`s both `statement-loader/requirements.txt` and `scripts/sql/finance_manager_db/requirements.txt` into it.
