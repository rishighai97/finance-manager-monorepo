...
Every module (`account-service`, `api-gateway`, `transaction-service`, `statement-loader`, `finance-manager-ui`, `dbscripts`, `test-automation`, `architecture-docs`, `observability`, `scripts`) keeps a `README.md` following this section structure. This file is the thing the docs-consistency skill/agent checks module READMEs against - if you add or rename a section, update both this file and that skill together.
