...
## Overview
A grab-bag of operational tooling for running the app locally and (previously) deploying it: starting Postgres, managing the DB schema, per-service local-run scripts, and some auth/SMTP infrastructure notes that aren't currently wired into the running app.

## Key modules
- `postgres.sh` - `docker run` command that starts the local Postgres container (`finance_manager` DB, port 5432).
- `sql/finance_manager_db/` - `database_manager.py`, a CLI for creating/dropping/truncating the app's Postgres tables from a JSON config (`finance_manager_db_config.py`); see its own README for usage.
