...
## Addendum - bulk-actions label (JIRA_15, 2026-09-12)

The batch-categorize row (flagged as missing from this spec in Implementation notes point 2) needed a short inline label indicating its buttons act in bulk on all visible transactions - `jira/JIRA_15.md` scoped this narrowly: placement was pre-decided (an inline prefix directly before the category-to-apply select), only the exact wording was open. Ran a quick wording-only round: 4 options ("Bulk:", "Apply to all:", "Batch:", "All visible:") mocked up in the real row context (including the row's "pending changes" state, since that's when "Revert Categorize"/"Update Categories" are visible alongside it - see JIRA_15's other half, which gates those two on `hasCategoryChanges`). User picked **"Bulk:"**.

**Implemented**: `<span class="bulk-label">Bulk:</span>` immediately before `.batch-category-input`, styled at the same muted micro-label weight as `.regex-label` (12px, `var(--app-color-muted)`, not the uppercase/letter-spaced summary-row treatment - too heavy next to four buttons). Verified live via `claude-in-chrome` (real in-app tab navigation, per the lesson from `ux/UX_transaction-search.md`'s Implementation notes) and via `ng test` (11/11 passing for `transaction-list`, including new characterization tests for the paired "Revert Categorize"/"Update Categories" visibility gate).
...
- 2026-09-12: addendum for `jira/JIRA_15.md`'s bulk-actions label - user picked "Bulk:" from 4 wording options; implemented as an inline prefix before the batch-categorize row's category select, verified build/tests/live (still Implemented)
