...
from model.grouped_user_account import GroupedUserAccount, GroupedUserAccountCustomJsonEncoder
...
@blueprint.route(rule="/fetch_all/grouped", methods=['GET'])
@cross_origin()
def fetch_all_grouped_accounts_by_user_ids() -> str:
    user_ids: List[int] = validate_and_get_user_ids()
    print(f"Received request to fetch grouped accounts for user ids {user_ids}")
    result = [record.__dict__ for record in service.get_all_grouped_accounts(user_ids=user_ids)]
    return json.dumps(result, cls=GroupedUserAccountCustomJsonEncoder)
